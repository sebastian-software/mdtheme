...
function validateFrame(value, index) {
    if (!isRecord(value)) {
    ...
    if (unknownKeys.length > 0) {
        throw new TypeError(`themes[${index}] has unknown key${unknownKeys.length === 1 ? "" : "s"}: ${unknownKeys.join(", ")}`);
    }
...
function validateConfigObject(config) {
    ...
    const unknownKeys = Object.keys(config).filter((key) => key !== "source" && key !== "output" && key !== "themes");
    if (unknownKeys.length > 0) {
        throw new TypeError(`unknown config key${unknownKeys.length === 1 ? "" : "s"}: ${unknownKeys.join(", ")}`);
    }
    if (!("themes" in config)) {
...
export function defineConfig(config) {
    ...
    const source = value.source === undefined ? DEFAULT_SOURCE : requireNonEmptyString(value.source, "source");
    const output = value.output === undefined ? DEFAULT_OUTPUT : requireNonEmptyString(value.output, "output");
    const themes = validateThemes(value.themes);
    return { source, output, themes };
}
function validateSourceName(value) {
    const sourceName = value === undefined ? DEFAULT_SOURCE : requireNonEmptyString(value, "sourceName");
...
function isSafeSourceName(sourceName) {
...
function validateRenderOptions(options) {
    ...
    if (unknownKeys.length > 0) {
        throw new TypeError(`unknown render option${unknownKeys.length === 1 ? "" : "s"}: ${unknownKeys.join(", ")}`);
    }
    return validateSourceName(options.sourceName);
}
function countTrailingNewlines(document) {
    let trailingNewlines = 0;
    for (let index = document.length - 1; index >= 0 && trailingNewlines < 2; index -= 1) {
        if (document.charCodeAt(index) !== 10)
            break;
        trailingNewlines += 1;
        if (index > 0 && document.charCodeAt(index - 1) === 13)
            index -= 1;
    }
    return trailingNewlines;
}
function addSectionBoundary(document) {
...
function joinSections(parts) {
    ...
    for (const part of parts) {
        if (part.length === 0)
            continue;
        if (hasSection)
            document = addSectionBoundary(document);
        document += part;
        hasSection = true;
    }
    return document;
 ...
 */
export async function renderMarkdown(source, themes, options) {
    ...
    const document = joinSections([
        `<!-- This file is generated by markdown-themer. Edit ${sourceName} instead. -->`,
        ...opening,
        source,
        ...closing,
    ]);

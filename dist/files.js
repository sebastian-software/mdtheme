...
async function validateDirectory(config) {
    ...
    try {
        ...
        if (error instanceof FileOperationError)
            throw error;
        throw new FileOperationError(`Cannot access output directory ${outputDirectory}: ${readableError(error)}`, { cause: error });
    }
}
async function validateSource(config) {
    try {
...
async function ensureDistinct(config, sourceStat) {
...
async function validateOutput(config, sourceStat) {
    const outputInfo = await lstatIfPresent(config.output);
    if (outputInfo === undefined)
        return { outputMode: 0o644, existing: undefined };
...
function validateOutputType(config, outputInfo) {
...
async function validateOutputDistinct(config, sourceStat) {
    try {
...
async function readOutput(config) {
    try {
...
export async function checkFiles(config) {
    const prepared = await prepare(config);
    return prepared.existing !== prepared.content;
}
async function writeAndClose(handle, content) {
    try {
...
async function atomicallyWrite(config, prepared) {
    const temporary = join(dirname(config.output), `.${basename(config.output)}.${process.pid}.${randomUUID()}.tmp`);
    ...
    try {
        ...
        if (temporaryOwned) {
            try {
                ...
                throw new FileOperationError(`Could not write ${config.output}: ${readableError(error)} (temporary cleanup failed: ${readableError(cleanupError)})`, { cause: cleanupError });
            }
        }
...
export async function writeFiles(config) {
    const prepared = await prepare(config);
    if (prepared.existing === prepared.content)
        return false;

...
async function git(cwd, args) {
    try {
        ...
        throw new Error("Cannot inspect Git state. Run pre-push inside a Git worktree with an existing commit and Git on PATH.", { cause: error });
    }
}
...
function repositoryPath(root, path) {
    ...
    if (isAbsolute(value) || value === ".." || value.startsWith(`..${sep}`)) {
        throw new Error("The pre-push config, source, and output must belong to the current Git worktree.");
    }
...
async function validateInputs(root, config, io) {
...
async function committedResult(root, head, output) {
...
export async function runPrePush(configPath, io) {
    const root = await realpath(await git(process.cwd(), ["rev-parse", "--show-toplevel"]));
    const head = await git(root, ["rev-parse", "--verify", "HEAD"]);
    if (await dirty(root)) {
        io.stderr("Push blocked: the worktree has staged, unstaged, or untracked changes. Commit or stash them, then push again. README generation was skipped.");
        return 1;
    }
    const config = await loadConfig(configPath);
    if (!(await validateInputs(root, config, io)))
        return 1;
    const changed = await writeFiles(config);
    if (changed)
        io.stdout(`Updated ${config.output}.`);
    const clean = await committedResult(root, head, config.output);
    if (changed || !clean) {
        io.stderr("Push blocked: review the generated README and Git changes, commit them, then push again. mdtheme did not stage or commit anything.");
        return 1;
    }

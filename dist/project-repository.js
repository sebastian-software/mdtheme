...
function normalizeRepository(value) {
    const raw = typeof value === "string" ? value : isRecord(value) ? stringValue(value.url) : undefined;
    if (raw === undefined)
        return undefined;
...
export function discoverRepository(root, npm, cargo) {
    const repository = normalizeRepository(npm?.repository) ??
...
export function discoverWorkflow(root) {
    const workflowRoot = resolve(root, ".github", "workflows");
    for (const fileName of ["ci.yml", "ci.yaml"]) {
        if (regularFile(resolve(workflowRoot, fileName)))
            return fileName;
    }
    let entries;
    try {
    ...
    const workflows = entries.filter((entry) => /\.ya?ml$/u.test(entry) && regularFile(resolve(workflowRoot, entry)));

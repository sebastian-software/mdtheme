...
The package requires Node.js 24 or newer. It is currently consumed from its
source repository, a local checkout, or a tarball rather than from npm:
...
For a committed Git dependency, pin the repository revision explicitly:
...
Replace the placeholder with the immutable commit selected for the
integration. The repository commits its built `dist/` files and does not rely
on a `prepare` lifecycle script. When the package is published, the same
dependency can be installed by name.
...
The [repository config](markdown-themer.config.ts) selects the company frame;
